...
def scale_bounds(bounding_box, window_dim, pct=0.05):
    ...
    new_bounding_box = (bounding_box[0]-woff, bounding_box[1]-hoff,
                        bounding_box[3]+woff, bounding_box[4]+hoff)

    b_ar = (new_bounding_box[3]-new_bounding_box[1]) / \
           (new_bounding_box[2]-new_bounding_box[0]) # New bb aspect ratio
    # Window aspect ratio
    ...
    try:
        ...
        logging.debug("Bounding box: %s - window: %s - hoff: %f - woff: %f - \
                       new_bounding_box: %s - b_ar: %f",
                      str(bounding_box), str(window_dim), hoff, woff, str(new_bounding_box), b_ar)
        raise exception

    if b_ar < s_ar: # Enlarge bb height to get the right AR - keep it centered vertically
        yoff = (new_bounding_box[3]-new_bounding_box[1])*(ratio-1)/2
        return (new_bounding_box[0], new_bounding_box[1]-yoff,
                new_bounding_box[2], new_bounding_box[3]+yoff)
    if b_ar > s_ar: # Enlarge window width to get the right AR - keep it centered horizontally
        xoff = (new_bounding_box[2]-new_bounding_box[0])*(1/ratio-1)/2
        return (new_bounding_box[0]-xoff, new_bounding_box[1],
                new_bounding_box[2]+xoff, new_bounding_box[3])
...
def linear_reg(x, y):
    ...
    rsquare = slope * math.sqrt((n_samples*sum_x2 - sum_x*sum_x) / \
...
def box_count(att_map, origin, box_side):
    ...
    for point in att_map.keys():
        box_c = tuple([int((coord_pt-coord_origin)/box_side)
...
def get_attractor_bounding_box(att):
    ...
    bounding_box = [[f(att_points, key=operator.itemgetter(i))[i] for i in range(0, att_dim)]
                    for f in (min, max)]
...
def compute_box_counting_dimension(att, scaling_factor=1.5):
    ...
    while divider < 256:
        ...
        for _ in range(0, box_count_trials): # Do something n times...
            origin = [random.randint(bounding_box[0][i], bounding_box[1][i])
...
def compute_correlation_dimension(att):
    ...
    for point in points: # Iterate on each attractor point
        other_point = points[random.randint(0, num_points-1)] # Pick another point at random
...
def compute_true_correlation_dimension(att, max_points=4096):
    ...
    logging.debug("Starting true correlation dimension computation on %d points.", num_points)
    ...
    for epsilon2, freq in bins.items():
        try:
            bins_log[math.log(math.sqrt(epsilon2))] = math.log(freq/num_points/num_points)
        except ValueError:
