...
class Attractor:
    ...
    def explore(self):
        ...
        """
        num = 0
        self.get_random_coef()
        while not self.check_convergence():
            num += 1
            self.get_random_coef()
        # Found one -> create corresponding code
        self.logger.debug("Attractor found after %d trials.", num+1)
        self.coef_to_code()
    ...
    def merge_attractors(self, attractor_pieces):
        ...
        used to merged pieces ofthe same attractor
    ...
    def create_frequency_map(self, window_geometry, nthreads):
        ...
        attractor_scaled_bb = util.scaleBounds(self.bound, window_geometry)
...
class PolynomialAttractor(Attractor):
    ...
    def compute_fractal_dimension(self, a_map):
        ...
        self.fdim = 0.0 if self.dimension == 3 else util.computeBoxCountingDimension(a_map)
...
class DeJongAttractor(Attractor):
    ...
    def compute_fractal_dimension(self, a_map):
        ...
        Work on the attractor map (using window coordinates)
        """
        self.fdim = min(2.0, util.computeBoxCountingDimension(a_map))
