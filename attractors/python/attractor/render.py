...
class Renderer(object):
    ...
    def getGradientSlice(self,s, n, grad_type='hsv_cw', out_space='hsv'):
    ...
    def getGradient(self, m, n, grad_type='hsv_cw', out_space='hsv'):
        g = list()
        for s in m:
            g += self.getGradientSlice(s, n, grad_type, out_space)
        return g
    ...
    def getPalette(self, frequencies):
        ...
        self.palette = dict()
        self.palette['background'] = tuple(reversed([round(component * ((1 << self.bpc)-1)) for component in rgb_norm(template['background'])]))
