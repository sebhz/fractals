...
def create_attractor(options):
    ...
    if options.type == "dejong":
        att = attractor.DeJongAttractor(
            iterations=int(options.iterations / options.threads), code=options.code
        )
    ...
    if options.code:
        if not att.check_convergence():
            logging.warning(
                "The specified attractor does not seem to converge. Bailing out."
            )
            sys.exit()
    else:
        att.explore()
    ...
    if options.dimension == 3:
        ...
        logging.debug(
            "Boundaries: (%.3f, %.3f) (%.3f, %.3f)", *(att.bound[0:2] + att.bound[3:5])
        )
    return att
...
def generate_attractor(geometry, options):
    ...
    logging.info(
        "Attractor type: %s %s",
        options.type,
        "(order = %d)" % (int(att.code[1]))
        if options.type == "polynomial"
        else "(symmetry = %d)" % (int(att.coef[5]))
...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--iterations", help="attractor number of iterations", type=int
    )
    parser.add_argument(
        "-n",
        "--number",
    ...
    parser.add_argument(
        "-p", "--png", help="save the attractor in a png file", action="store_true"
    )
    parser.add_argument(
        "-P",
        "--palette",
    ...
    parser.add_argument(
        ...
        type=str,
        choices=("polynomial", "dejong", "clifford", "icon"),
    )
...
if ARGS.iterations is None:
    ...
    logging.warning(
        "For better rendering, you should use at least %d iterations.", IDEAL_ITER
    )
