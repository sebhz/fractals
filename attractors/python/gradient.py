...
for i, template in enumerate(palettes.pal_templates):
    hsv_gradient = palettes.getGradient(template['gradient_map'],
                                        NUM_COLORS,
                                        template['colorspace'],
                                        'hsv')
    map_gradient(hsv_gradient, NUM_COLORS, template['value_offset'], template['invert_value'])
    rgb_gradient = [tuple([round(255*component) for component in colorsys.hsv_to_rgb(*color)])
