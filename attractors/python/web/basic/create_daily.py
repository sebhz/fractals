...
def get_filename(code):
    ...
    if len(code) < max_fname_length - len(IMAGE_SUFFIX) - PATH_GUARDBAND:
        ...
        fname = (
            code[: max_fname_length - len(IMAGE_SUFFIX) - PATH_GUARDBAND - 1]
            + "#"
            + IMAGE_SUFFIX
        )
    return fname
...
def write_attractors(keywords_map, img, args):
    ...
    for img_def in (
        ...
        if not os.path.exists(rooted_target_dir):
            ...
            logging.error(
                "Output directory %s exists, but is a plain file. Ignoring it.",
                rooted_target_dir,
            )
            continue
...
def remove_images(keywords_map, args):
    ...
    for directory in ("png", "png_thumb", "png_tile"):
        ...
        if not os.path.exists(rooted_target_dir) or not os.path.isdir(
            rooted_target_dir
        ):
            continue
